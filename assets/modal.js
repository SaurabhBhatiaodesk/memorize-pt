class Modal extends HTMLElement {
  ...
  static close_button = document.querySelector(".layout--close");
  ...
  connectedCallback() {
    !this.blocking ||
      Shopify.designMode ||
      (window.localStorage &&
        void 0 !== localStorage[theme.local_storage[this.blocking]]) ||
      this.open();
  }
  ...
  open() {
    ...
    if(typeof(checkCanvasZoom)=="function")
    checkCanvasZoom();
  ...
  close() {
    "closed" === Modal.status ||
      ...
      this.blocking
        ? (localStorage[theme.local_storage[this.blocking]] = "true")
...
(theme.modal = Modal), customElements.define("modal-root", Modal);
      ...
      this.on(
        ...
        (t) => {
          if ("keydown" !== t.type || "Enter" === t.key) {
            const i = theme.modal.instances[this.dataset.id];
            var e = (i.trigger = this).getAttribute("data-index");
            e && i.toggleView(parseInt(e), !1),
              "closed" !== theme.drawer.status && theme.drawer.close(),
              isNaN(this.delay)
                ? i.open()
                : setTimeout(() => i.open(), this.delay);
          }
        }
      );
  }
...
(theme.modal = Modal), customElements.define("modal-trigger", ModalTrigger);





function checkCanvasZoom()
  {
    var canvasElement = document.querySelector('.product-media--root[data-active="true"] canvas#pplr_canvas');
    if(window.innerWidth > 767 && canvasElement)
    {
      var imageSrc = canvasElement.toDataURL(); 
      var imageElement = document.querySelector('.modal--view.canvasZoom img');
      var modalElement = document.querySelector('.product-page--desktop-container modal-root.modal--root[data-id="product"]');
      if(modalElement && imageElement){
        modalElement.classList.add('canvasZoom');
